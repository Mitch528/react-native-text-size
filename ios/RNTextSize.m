                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    const BOOL includeAllLines = [options[@"includeAllLines"] boolValue];
    ...
    if (includeAllLines) {
        ...
        for (int line = 0; line < lineCount; line++) {
            ...
            if (lineInfo) {
                [lines addObject:lineInfo];
            }
        }
        
        [result setValue:[NSArray arrayWithArray:lines] forKey:@"lineInfo"];
    } else if (!isnan(optLine) && optLine >= 0) {
...
{
    const UIFontDescriptorSymbolicTraits symbolicTraits = [traits[UIFontSymbolicTrait] unsignedIntValue];
    const BOOL isItalic = (symbolicTraits & UIFontDescriptorTraitItalic) != 0;
